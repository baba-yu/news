...
5. **Leave `tech.agent_control_plane` and `tech.one_bit_edge_llm` shape alone**, only edit description on `tech.agent_control_plane`. Their child counts will fall once Physical AI moves out and the description pruning lands.

(No category changes; no theme deletions.)
