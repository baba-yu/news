...
5. **Leave `tech.agent_control_plane` and `tech.one_bit_edge_llm` shape alone**, only edit description on `tech.agent_control_plane` to drop generic "agent" / "robot" wording so the matcher stops absorbing Physical AI into it. Both themes are at their right granularity once Physical AI moves out — no split needed this week.
