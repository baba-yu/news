...
Pre-condition: `app/update_pages.bat` has run at least once for today (or today's data is already up to date in `docs/data/graph-*.json` and `app/data/analytics.sqlite`). If not, run it first or proceed with yesterday's snapshot — the spec is robust to a one-day-stale view.
...
Filename: `memory/theme-review/theme-review-YYYYMMDD.md`. Create the `memory/theme-review/` folder if it does not exist (alongside `memory/dormant/`).
