...
## Step 1. Read bounded inputs

Read **only** these files (do not scan history beyond them):

- Previous dormant snapshot `memory/dormant/dormant-{previous Sunday YYYYMMDD}.md` (1 file).
...
## Step 2. Compute tier transitions
...
## Step 3. Extract Signals for new dormant entrants
...
If all dormant predictions were revived this week, still emit the file with the header and an empty table — the file's existence is the signal that the weekly job ran.
...
| | Files | Notes |
|---|---|---|
| Read | 1 previous dormant snapshot |  |
...
Total max: 15 reads, 1 write. Constant in project age.

---

## Failure modes

- **Previous dormant snapshot malformed** → log the file path and the parse error, abort. Resolve the previous snapshot before retrying. Do not silently drop entries.
